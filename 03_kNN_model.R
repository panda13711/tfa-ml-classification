...
cat("k-NEAREST NEIGHBORS (kNN) MODEL\n")
cat(paste(rep("=", 80), collapse=""), "\n\n")
...
knn_model <- train(
  ...
  tuneGrid = expand.grid(k = c(3, 5, 7, 9, 11, 15, 21))
)
...
write.csv(results, "results_kNN.csv", row.names=FALSE)
cat("Results saved to results_kNN.csv\n\n")
