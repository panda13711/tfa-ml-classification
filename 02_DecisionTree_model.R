...
model_data <- data.frame(
  ...
  IsStemMajor = factor(data$`Is Math, Sci, or Eng Major Minor`),
  ...
  RegionPrefLevel = factor(data$`Region Preference Level`),
  AttendedEvent = factor(data$`Attended Event`),
  Met = factor(data$Met),
  Invited = factor(data$Invited),
  Responded = factor(data$Responded),
  CompletedAdmissions = factor(data$`Completed Admissions Process`),
...
cat("\n")
cat(paste(rep("=", 80), collapse=""), "\n")
cat("SUMMARY - DECISION TREE MODEL RESULTS\n")
cat(paste(rep("=", 80), collapse=""), "\n")
...
write.csv(results, "results_DecisionTree.csv", row.names=FALSE)
cat("Results saved to results_DecisionTree.csv\n\n")
